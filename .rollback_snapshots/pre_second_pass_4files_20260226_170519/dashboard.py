...
class DashboardPage(QWidget):
    ...
    def _build(self):
        ...
        hero.setStyleSheet("""
            QFrame {
                background: qlineargradient(x1:0,y1:0,x2:1,y2:1,
                    stop:0 rgba(62,240,176,0.06),
                    stop:0.5 rgba(167,139,250,0.05),
                    stop:1 rgba(59,130,246,0.04));
        ...
        self._device_sub_lbl  = lbl("Checking mesh, KDE Connect, and sync services…",
        ...
        self._hero_state_dot = QFrame()
        self._hero_state_dot.setFixedSize(8, 8)
        self._hero_state_dot.setStyleSheet(f"background:{TEAL};border:none;border-radius:4px;")
        breathe(self._hero_state_dot, min_opacity=0.35, max_opacity=1.0)
        dot_row.addWidget(self._hero_state_dot)
        self._hero_state_lbl = lbl("Connected", 10, TEAL, mono=True)
        dot_row.addWidget(self._hero_state_lbl)
        dot_row.addStretch()
        status_col.addLayout(dot_row)
        status_col.addWidget(lbl("Live service state", 10, TEXT_DIM, mono=True))
        top.addLayout(status_col)
        hl.addLayout(top)

        # Pills
        pills_row = QHBoxLayout()
        pills_row.setSpacing(8)
        self._kde_pill = pill("KDE Connect", TEAL, pulse=True)
        self._ts_pill = pill("Tailscale · mesh", VIOLET, pulse=True)
        ...
        cl.setSpacing(0)
        self._ts_row = self._conn_row("🔗","Tailscale VPN",
                                    "Checking…", True, TEAL,
                                    on_toggle=self._toggle_tailscale_action)
        cl.addWidget(self._ts_row)
        cl.addWidget(divider())
        self._kde_row = self._conn_row("📶","KDE Connect",
                                    "Checking…", True, TEAL,
                                    on_toggle=self._toggle_kde_action)
        cl.addWidget(self._kde_row)
        cl.addWidget(divider())
        ...
        cl.addWidget(divider())
        self._hotspot_row = self._conn_row("📡","Mobile Hotspot",
                                    "Off · toggle via ADB", False, CYAN,
                                    on_toggle=self._hotspot)
        cl.addWidget(self._hotspot_row)
    ...
    def _set_conn_state(self, row, connected, text):
        if hasattr(row, "_sub"):
            row._sub.setText(text)
        if hasattr(row, "_dot"):
            color = TEAL if connected else ROSE
            row._dot.setStyleSheet(f"background:{color};border:none;border-radius:3px;")
            if connected:
                breathe(row._dot, min_opacity=0.35, max_opacity=1.0)
            else:
                anim = getattr(row._dot, "_pb_breathe_anim", None)
                if anim is not None:
                    anim.stop()
                    row._dot._pb_breathe_anim = None
                effect = row._dot.graphicsEffect()
    ...
    def _apply_refresh(self, data, include_media=False):
        self._refresh_busy = False
        ts_connected = bool((data or {}).get("tailscale"))
        ts_ip = (data or {}).get("tailscale_ip") or "offline"
        kde_enabled = bool((data or {}).get("kde_enabled", True))
        kde_reachable = bool((data or {}).get("kde_reachable"))
        syncthing_ok = bool((data or {}).get("syncthing"))

        if ts_connected and kde_reachable:
            self._hero_state_dot.setStyleSheet(f"background:{TEAL};border:none;border-radius:4px;")
            breathe(self._hero_state_dot, min_opacity=0.3, max_opacity=1.0)
            self._hero_state_lbl.setText("Connected")
            self._hero_state_lbl.setStyleSheet(f"color:{TEAL};font-size:10px;font-family:monospace;background:transparent;border:none;")
        else:
            self._hero_state_dot.setStyleSheet(f"background:{ROSE};border:none;border-radius:4px;")
            self._hero_state_lbl.setText("Attention")
            self._hero_state_lbl.setStyleSheet(f"color:{ROSE};font-size:10px;font-family:monospace;background:transparent;border:none;")
        self._device_sub_lbl.setText(
            f"{ts_ip} · KDE {'reachable' if kde_reachable else ('disabled' if not kde_enabled else 'offline')} · "
            f"Syncthing {'running' if syncthing_ok else 'stopped'}"
        )
        self._set_pill(
            self._kde_pill,
            f"KDE Connect · {'Live' if kde_reachable else ('Disabled' if not kde_enabled else 'Offline')}",
            TEAL if kde_reachable else ROSE,
            pulse=kde_reachable,
        )
        self._set_pill(
            self._ts_pill,
            f"Tailscale · {ts_ip if ts_connected else 'Offline'}",
            VIOLET if ts_connected else ROSE,
            pulse=ts_connected,
        )
        self._set_pill(
            self._sync_pill,
            f"Syncthing · {'On' if syncthing_ok else 'Off'}",
            CYAN if syncthing_ok else ROSE,
            pulse=syncthing_ok,
        )
        ...
        self._set_conn_state(
            self._ts_row,
            ts_connected,
            f"{ts_ip} · mesh {'active' if ts_connected else 'offline'}",
        )
        self._set_conn_state(
            self._kde_row,
            kde_reachable and kde_enabled,
            "LAN + Tailscale fallback" if kde_reachable else ("Disabled in settings" if not kde_enabled else "Not reachable"),
        )
        if hasattr(self._ts_row, "_toggle"):
            self._ts_row._toggle.blockSignals(True)
            self._ts_row._toggle.setChecked(ts_connected)
            self._ts_row._toggle.blockSignals(False)
        if hasattr(self._kde_row, "_toggle"):
            self._kde_row._toggle.blockSignals(True)
            self._kde_row._toggle.setChecked(kde_enabled)
            self._kde_row._toggle.blockSignals(False)
    ...
    def _set_pill(self, widget, text, color, pulse=False):
        if widget is None:
            return
        ...
        if dot is not None:
            ...
            if pulse:
                ...
                anim = getattr(dot, "_pb_breathe_anim", None)
                if anim is not None:
                    anim.stop()
                    dot._pb_breathe_anim = None
    ...
    def _toggle_tailscale_action(self, checked=False):
        target = bool(checked)
        ok = self.ts.set_enabled(target)
        push_toast(
            "Tailscale connected" if target else "Tailscale disconnected",
    ...
    def _toggle_kde_action(self, checked=False):
        ...
        push_toast(
            "KDE Connect enabled" if target else "KDE Connect disabled",
            "success",
            1700,
        )
