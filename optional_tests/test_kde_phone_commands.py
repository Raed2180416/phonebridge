...
def test_installer_writes_plugin_flag(tmp_path):
    ...
    cfg = tmp_path / ".config" / "kdeconnect" / "a9fe30c209da40d4bddce484a2c4112a" / "config"
...
def test_installer_writes_runcommand_config_with_expected_ids_and_names(tmp_path):
    res = _run_installer(tmp_path)
    assert res.returncode == 0
    cfg = tmp_path / ".config" / "kdeconnect" / "a9fe30c209da40d4bddce484a2c4112a" / "kdeconnect_runcommand" / "config"
    text = cfg.read_text(encoding="utf-8")
...
def test_remove_only_deletes_runcommand_config(tmp_path):
    ...
    cfg = tmp_path / ".config" / "kdeconnect" / "a9fe30c209da40d4bddce484a2c4112a" / "config"
    run_cfg = tmp_path / ".config" / "kdeconnect" / "a9fe30c209da40d4bddce484a2c4112a" / "kdeconnect_runcommand" / "config"
    assert cfg.exists()
