...
_ORIG_BACKEND_SETTINGS = sys.modules.get("backend.settings_store")
_ORIG_BACKEND_ADB = sys.modules.get("backend.adb_bridge")
...
if _ORIG_BACKEND_SETTINGS is None:
    sys.modules.pop("backend.settings_store", None)
else:
    sys.modules["backend.settings_store"] = _ORIG_BACKEND_SETTINGS
if _ORIG_BACKEND_ADB is None:
    sys.modules.pop("backend.adb_bridge", None)
else:
    sys.modules["backend.adb_bridge"] = _ORIG_BACKEND_ADB
...
def test_watchdog_starts_on_active_result(monkeypatch):
    ...
    ar._sources["call_pc_active"] = True
